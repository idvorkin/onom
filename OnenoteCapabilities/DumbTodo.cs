...
namespace OnenoteCapabilities
{
    public class DumbTodo
    {
        ...
        public static void AddToPage(XDocument pageContentAsXML, string todo, DateTime? dueDate=null, int tableOnPage=0)
        {
            AddTodoTagToPageIfRequired(pageContentAsXML);
        ...
        private static void AddTodoTagToPageIfRequired(XDocument pageContentAsXml)
        {
        ...
        public static void AddToPageFromDateEnableSmartTag(XDocument pageContent, SmartTag smartTag, int tableOnPage=0)
        {
            var parsedDate = HumanizedDateParser.ParseDateAtEndOfSentance(smartTag.TextAfterTag());

            // HACK - TBD Design this.
            var smartTodoPrefix = SmartTodo.CreateSmartTodoLink(smartTag);

            if (parsedDate.Parsed)
            {
                DumbTodo.AddToPage(pageContent, smartTodoPrefix+parsedDate.SentanceWithoutDate, parsedDate.date, tableOnPage);
            }
            else
            {
                DumbTodo.AddToPage(pageContent, smartTodoPrefix+smartTag.TextAfterTag(), tableOnPage:tableOnPage);
            }
        }
    }
}
